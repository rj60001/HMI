<?php
  if(isset($_GET["sequence"])){ # If we are retriving a specific sequence.
    ...
    $r = mysqli_query($db, "SELECT histoneMods, ndsid FROM nucleosome WHERE nsid=$nsid"); # Fetch data on all nucleosome related to this sequence.
    $counter = 0;
    while($row = mysqli_fetch_array($r)){
      ...
      for($i=0;$i<count($mods);$i++){
        $mod = $mods[$i]; # Select the current hisotne modification ID (hmid).
