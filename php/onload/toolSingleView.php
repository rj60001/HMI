  ...
  if(isset($_GET["sequence"])){ # If we are retriving a specific sequence.
    ...
    while($row = mysqli_fetch_array($r)){
      ...
      if($uid == $ownerid){ # Checks to see if the user owns this sequence.
        $html .= '<form action="index.php?page=tool&sequence='.$nsid.'" method="post"><input name="nidTD" type="hidden" value="'.$nid.'"/><input name="submitTD" type="submit" value="Delete" onfocus="selected(this);" onblur="deselected(this);"/></form>';
      }
      $html .= '</div><br><br>';
    ...
    while($row = mysqli_fetch_array($r)){
      ...
      if($uid == $ownerid){ # Checks to see if the user owns this sequence.
        echo('<form action="index.php?page=tool&disease='.$did.'" method="post"><input name="nsidTD" type="hidden" value="'.$nsid.'"/><input name="submitTD" type="submit" value="Remove" onfocus="selected(this);" onblur="deselected(this);"/></form>');
      }
      echo('</div><br><br>');
    }
