<?php
	...
	$q = "SELECT tid, subject, uid, dateTime FROM thread ORDER BY tid DESC"; # Fetches all threads.
	$r = mysqli_query($db, $q);
	...
	while($row = mysqli_fetch_array($r)){ # For each thread.
		...
		echo("<div class='strip greenYellow floatAesthetic' onclick='window.location.href = `index.php?page=forum&thread=$row[0]`;'><a>$s</a><hr><br><a style='float: right;'>$fn at ".$row[3]."</a>");
		# Each thread displayed as a strip with a link to the forum sing view so that we can view a single thread.
		if($admin == TRUE){ # If the user is an admin, they should have the power to delete inappropriate threads. Displaya  from that lets them do that.
	...
	if(mysqli_num_rows($r) < 1){ # If no threads exist, print a message to indicate this to the user.
	...
	if(isset($_GET["thread"]) && isset($_COOKIE["user"])){ # If we are in are viewing a single thread on forumSingleViewPage.
		...
		echo("<br><br><div class='forumObj greenYellow floatAesthetic'><div class='forumCon'><b>$rowT[0]</b><br><t>$m</t><br><em class='forumNameTag'>$rowT[2] at $rowT[3]</em></div></div>"); # Displays the original post.
		$r = mysqli_query($db, "SELECT message, firstName, mid, dateTime FROM message INNER JOIN users ON message.uid = users.uid WHERE tid=".$tid); # Fetch data on all messages to the original post including the first name of the use rwho posted it.
		while($rowM = mysqli_fetch_array($r)){ # Foreach message.
			$m = convert($rowM[0]);
			...
			while($rowR = mysqli_fetch_array($rR)){
				...
				#Display the reply.
				if($admin == TRUE){ # Dispaly a form to delete the replie if the user is an admin.
