  ...
  class dictionary{
    public function dictionary($keyValues = []){ # This constructor is called when a dictionary object is instantiated.
      ...
      for($i=0;$i<count($keyValues);$i++){
        if(gettype($i/2) == "double"){ # If integer is not formed when divided by two, must be an odd number.
    ...
    public function add($key, $value){
      $val = dictionary::getKeyPosition($key);
      if($val == -1){ # If the key does not already exist, then a new key can be added.
